...
Papers Empire is a browser-based incremental game that runs entirely on the client side. The experience is intentionally light-hearted: you're modernising an industrial print shop while channeling the vibe of workplace sitcoms. The current release contract is **0.23.1 — Quality Pass V5**.
...
- PNG icons (`favicon-32`, `apple-touch-icon` 180, `icon-192`, `icon-512`) are derived from the deterministic Production Twin master `assets/brand/papers-empire-app-mark-v2.svg`; `site.webmanifest` makes the game installable.
